...
generate_tilted_data <- function(csv_path,
  ...
  # 7. Apply Shift with Subsampling
  test_pool_scores <- test_pool_mat[, top_covs, drop = FALSE] %*% matrix(current_tilt_coefs, ncol = 1)
  
  # Re-apply numerical stability for weights calculation
  shifted_test_scores <- gamma_opt * test_pool_scores
  test_weights <- exp(shifted_test_scores - max(shifted_test_scores))
  
  # Clip weights to prevent zero-probability sampling if weights are extremely skewed 
  test_weights[test_weights < 1e-10] <- 1e-10
  
  new_size <- floor(nrow(data_test_pool_raw) * subsample_ratio)
  shifted_indices <- sample(
    seq_len(nrow(data_test_pool_raw)),
    size = new_size,
    replace = TRUE,
    prob = test_weights
  )
  
  vroom_write(data_test_pool_raw[shifted_indices, ], output_path, delim = "\t")
...
generate_tilted_data(
  ...
  subsample_ratio = as.numeric(snakemake@params[["subsample_ratio"]]),
